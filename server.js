...
app.put("/professors/:pid", async (req, res, next) => {
	try {
		...
		if (professor) {
			...
			res.status(202).json({
				message: "Professor with ID:" + req.params.pid + " has been succesfully modified.",
			});
		} else {
...
app.delete("/professors/:pid", async (req, res, next) => {
	try {
		...
		if (professor) {
			...
			res.status(202).json({
				message: "Professor with ID:" + req.params.pid + " has been succesfully deleted.",
			});
		} else {
			res.status(404).json({ message: "Professor not found." });
		}
	...
	try {
		...
		if (professor) {
			...
			if (student) {
				...
				res.status(202).json({
					message: "Student with ID:" + req.params.sid + " has been succesfully modified.",
				});
			} else {
				res.status(404).json({
					message: "Student with ID:" + req.params.sid + " was not found.",
	...
	try {
		...
		if (professor) {
			...
			if (student) {
				...
				res.status(202).json({
					message: "Student with ID:" + req.params.sid + " has been succesfully deleted.",
				});
			} else {
				res.status(404).json({
					message: "Student with ID:" + req.params.sid + " was not found.",
